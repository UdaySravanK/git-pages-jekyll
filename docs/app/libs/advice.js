...
// work derived from https://github.com/twitter/flight/blob/master/libs/advice.js
// ==========================================
  ...
  Backbone.Advice = {
    ...
    setDefaults: function(obj) {
      ...
      if (typeof base == 'function')
        base = this.prototype;
      _.each(obj, function(val, key) {
    	 //dont need to search in the prototype chain 
    	//need to apply the mixin if function is not available at that level	
    	 // example case: if mixin function was common to   phone, tablet, desktop
    	 //	if there is a specific change we only modify that file
    	 //    	  if (!Backbone.Advice.findVal(base, key))   --- orginal code
    	 //	  base[key] = val;  --- orginal code
    	
    	  //start new code, check anscestor only if option to check in hierarchy is provided
